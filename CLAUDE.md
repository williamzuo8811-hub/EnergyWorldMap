...
in **`js/products-meta.js`** (`window.ENERGY_PRODUCTS = { PRODUCTS, match }`) — three core product lines (模块化预制舱
变电站 / 车载移动变电站 / 算电岛 AI PowerHouse 算力中心高压交直流预制舱供电站), each with a `cats`/`kw` rule. `match(p)` derives the applicable
