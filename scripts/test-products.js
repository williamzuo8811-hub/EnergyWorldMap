...
/* ---------- 目录结构 ---------- */
ok(Array.isArray(P.PRODUCTS) && P.PRODUCTS.length === 3, 'PRODUCTS 恰为 3 个核心产品');
ok(P.PRODUCTS.map(pr => pr.key).join(',') === 'prefab,mobile,aipower', '产品为 预制舱变电站 + 车载移动变电站 + 算电岛 AI PowerHouse');
...
/* ---------- match：不再命中已下线的产品（E-house / 中低压成套 / 储能并网舱）---------- */
...
ok(P.match({ cat: 'storage', name: '电池储能电站', desc: '' }).length === 0, '纯储能（无变电/移动/算力关键词）→ 无适配产品');
